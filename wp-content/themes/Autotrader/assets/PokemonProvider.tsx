...
interface PokemonContextData {
  ...
  togglePokemonType: (type: PokemonType) => void
  selectedPokemonTypes: Array<string>
}
...
const pokemonTypesMapping: Record<string, string[]> = {}
...
const PokemonProvider: React.FC<PokemonProviderProps> = ({ children }) => {
  ...
  const [selectedPokemonTypes, setSelectedPokemonTypes] = useState<Array<string>>([])
  ...
  useEffect(() => {
    filterData()
  }, [filters, query, data, selectedPokemonTypes])
  ...
  const filterData = async () => {
    ...
    if (selectedPokemonTypes.length > 0) {
      filteredData = filteredData
        .filter(({ name }) => selectedPokemonTypes
          .some((type) => pokemonTypesMapping[name].includes(type))
        )
    }
  ...
  const fetchPokemon = async () => {
    try {
      ...
      await Promise.all(response.results.map(async ({ name }) => {
        const { types } =  await PokeAPI.Pokemon.resolve(name)
        pokemonTypesMapping[name] = types.map(({ type }) => type.name)
      }))
  ...
  function togglePokemonType (type: PokemonType) {
    if (!selectedPokemonTypes.includes(type)) {
      ...
      setSelectedPokemonTypes(selectedPokemonTypes.filter((t) => t !== type))
    }
  }
